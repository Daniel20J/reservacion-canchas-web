    ...
    <section className="management-page reservations-page">
      <div className="page-header">
        <div>
          ...
          <h1>Reservaciones</h1>
          <p>Control de horarios, clientes y disponibilidad.</p>
        </div>
        <button className="primary-button page-action" type="button" onClick={abrirNueva}>
          <Plus size={19} />Nueva reservación
        </button>
      </div>
      ...
      <form className="reservation-filters panel" onSubmit={buscar}>
        <label>
          <span>Fecha</span>
          <input type="date" value={filtros.fecha} onChange={(event) => setFiltros((actual) => ({ ...actual, fecha: event.target.value }))} />
        </label>
        <label>
          <span>Cancha</span>
          <select value={filtros.idCancha} onChange={(event) => setFiltros((actual) => ({ ...actual, idCancha: event.target.value }))}>
            <option value="">Todas las canchas</option>
            {canchas.map((cancha) => (
              <option key={cancha.idCancha} value={cancha.idCancha}>{cancha.nombre}</option>
            ))}
          </select>
        </label>
        <label>
          <span>Estado</span>
          <select value={filtros.estado} onChange={(event) => setFiltros((actual) => ({ ...actual, estado: event.target.value }))}>
            <option value="">Todos los estados</option>
            {estados.map((estado) => (
              <option key={estado.idEstadoReservacion} value={estado.nombre}>{estado.nombre}</option>
            ))}
          </select>
        </label>
      ...
      <section className="panel reservation-list">
        <div className="panel__header">
          <div>
            ...
            <p>{cargando ? 'Consultando información...' : `${reservaciones.length} resultado${reservaciones.length === 1 ? '' : 's'}`}</p>
          </div>
          <span className="total-badge">{reservaciones.length} total</span>
        </div>
            ...
            <tbody>
              {cargando ? (
                <tr>
                  <td colSpan="8">
                    <div className="table-message">
                      <LoaderCircle className="spin" size={24} />Cargando reservaciones...
                    </div>
                  </td>
                </tr>
                ...
                <tr key={reservacion.idReservacion}>
                  ...
                  <td>
                    <strong>{reservacion.cancha}</strong>
                    <small>{reservacion.tipoCancha}</small>
                  </td>
                  ...
                  <td>
                    <div className="reservation-actions">
                      {reservacion.estado === 'RESERVADA' && (
                        <>
                          <button type="button" onClick={() => abrirEdicion(reservacion)} disabled={cargandoEdicion} title="Editar">
                            <Edit3 size={16} /><span>Editar</span>
                          </button>
                          <button className="use" type="button" onClick={() => cambiarEstado(reservacion, 'utilizar')} title="Marcar utilizada">
                            <CheckCircle2 size={17} /><span>Utilizar</span>
                          </button>
                          <button className="cancel" type="button" onClick={() => cambiarEstado(reservacion, 'cancelar')} title="Cancelar">
                            <Ban size={17} /><span>Cancelar</span>
                          </button>
                        </>
                      )}
                    </div>
                ...
                <tr>
                  <td colSpan="8">
                    <div className="table-message empty">
                      <CalendarDays size={28} />No hay reservaciones para los filtros seleccionados.
                    </div>
                  </td>
                </tr>
              )}
        ...
        <form className="entity-form reservation-form" onSubmit={guardar}>
          <div className="form-grid">
            <label className="form-field">
              <span>Cliente *</span>
              <select name="idCliente" value={formulario.idCliente} onChange={actualizarCampo} required autoFocus>
            ...
            <label className="form-field">
              <span>Cancha *</span>
              <select name="idCancha" value={formulario.idCancha} onChange={actualizarCampo} required>
                <option value="">Selecciona una cancha</option>
                {canchas.map((cancha) => (
                  <option key={cancha.idCancha} value={cancha.idCancha}>{cancha.nombre} · {cancha.tipo}</option>
                ))}
              </select>
            </label>
            <label className="form-field">
              <span>Fecha y hora de inicio *</span>
              <input name="fechaInicio" type="datetime-local" value={formulario.fechaInicio} onChange={actualizarCampo} required />
            </label>
            <label className="form-field">
              <span>Fecha y hora de fin *</span>
              <input name="fechaFin" type="datetime-local" value={formulario.fechaFin} onChange={actualizarCampo} required />
            </label>
            <label className="form-field full">
              <span>Observaciones</span>
              <textarea name="observaciones" value={formulario.observaciones} onChange={actualizarCampo} maxLength="500" rows="3" placeholder="Información adicional de la reservación" />
            </label>
          </div>

          {!modal.id && disponibilidad && (
            <div className={`availability ${disponibilidad.consultando ? 'checking' : disponibilidad.disponible ? 'available' : 'unavailable'}`}>
          ...
          <div className="estimate-box">
            <div>
              <span><Clock3 size={17} />Duración estimada</span>
              <strong>{numero.format(estimacion.horas)} h</strong>
            </div>
            <div>
              <span><CircleDollarSign size={17} />Precio por hora</span>
              <strong>{dinero(estimacion.precio)}</strong>
            </div>
            <div>
              <span><CircleDollarSign size={17} />Total estimado</span>
              <strong>{dinero(estimacion.total)}</strong>
            </div>
            <p>Estimación informativa. El cálculo definitivo lo realiza el servidor.</p>
          </div>

          <div className="form-actions">
            <button className="secondary-button" type="button" onClick={() => setModal({ abierto: false, id: null })}>
              Cancelar
            </button>
