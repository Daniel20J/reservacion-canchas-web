    ...
    <section className="management-page users-page">
      <div className="page-header">
        <div>
          ...
          <h1>Usuarios</h1>
          <p>Administración de acceso y roles.</p>
        </div>
      ...
      <section className="panel management-panel">
        <div className="panel__header">
          <div>
            ...
            <p>{cargando ? 'Consultando información...' : `${usuarios.length} usuario${usuarios.length === 1 ? '' : 's'} registrado${usuarios.length === 1 ? '' : 's'}`}</p>
          </div>
          <span className="total-badge">{usuarios.length} total</span>
        </div>
            ...
            <tbody>
              {cargando ? (
                <tr>
                  <td colSpan="5">
                    <div className="table-message">
                      <LoaderCircle className="spin" size={24} />Cargando usuarios...
                    </div>
                  </td>
                </tr>
                ...
                <tr key={usuario.idUsuario}>
                  <td>
                    <div className="client-cell user-cell">
                      <span>{usuario.nombreCompleto.charAt(0).toUpperCase()}</span>
                  ...
                  <td>
                    <div className="role-list">
                      {usuario.roles.map((rol) => (
                        <span className={`role-badge role-badge--${rol.toLowerCase()}`} key={rol}>{rol}</span>
                      ))}
                    </div>
                  </td>
                  <td>
                    <span className={`entity-status ${usuario.activo ? 'is-active' : 'is-inactive'}`}>
                      {usuario.activo ? 'ACTIVO' : 'INACTIVO'}
                    </span>
                  </td>
                  <td>
                    <div className="row-actions user-actions">
                      <button className="icon-action" type="button" onClick={() => abrirModal(usuario)} title="Editar usuario">
                        <Edit3 size={17} /><span>Editar</span>
                      </button>
                      <button className="icon-action" type="button" onClick={() => cambiarPassword(usuario)} title="Cambiar contraseña">
                        <KeyRound size={17} /><span>Contraseña</span>
                      </button>
                      <button className={`icon-action ${usuario.activo ? 'danger' : 'success'}`} type="button" onClick={() => cambiarEstado(usuario)} title={usuario.activo ? 'Desactivar usuario' : 'Activar usuario'}>
                        {usuario.activo ? <ToggleRight size={19} /> : <ToggleLeft size={19} />}
                ...
                <tr>
                  <td colSpan="5">
                    <div className="table-message empty">
                      <UsersRound size={28} />No hay usuarios registrados.
                    </div>
                  </td>
                </tr>
              )}
        ...
        <form className="entity-form" onSubmit={guardar}>
          <div className="form-grid">
            <label className="form-field full">
              ...
              <input name="nombreCompleto" value={formulario.nombreCompleto} onChange={actualizarCampo} maxLength="150" required autoFocus />
            </label>
            <label className="form-field full">
              <span>Correo electrónico *</span>
              <input name="correo" type="email" value={formulario.correo} onChange={actualizarCampo} required disabled={Boolean(modal.usuario)} />
            </label>
            {!modal.usuario && (
              <label className="form-field full">
                <span>Contraseña *</span>
                <input name="password" type="password" value={formulario.password} onChange={actualizarCampo} minLength="8" required autoComplete="new-password" placeholder="Mínimo 8 caracteres" />
              </label>
            )}
            <label className="form-field">
              <span>Rol *</span>
              <select name="rol" value={formulario.rol} onChange={actualizarCampo} required>
                {roles.map((rol) => (
            ...
            {modal.usuario && (
              <label className="form-field">
                <span>Estado *</span>
                <select name="activo" value={String(formulario.activo)} onChange={actualizarCampo} required>
          ...
          <div className="form-actions">
            <button className="secondary-button" type="button" onClick={() => setModal({ abierto: false, usuario: null })}>
              Cancelar
            </button>
